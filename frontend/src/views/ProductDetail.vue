<template>
  <div class="product-detail" v-loading="loading">
    <el-page-header @back="$router.go(-1)" title="Back" class="mb-4">
      <template #content>
        <span class="text-large font-600 mr-3"> Product Details </span>
      </template>
    </el-page-header>
    ...
    <div v-if="product" class="detail-container">
      ...
      <div class="product-card">
        ...
        <div class="product-info">
          <h1>{{ product.name }}</h1>
          <el-tag type="info" style="margin-bottom: 15px;">{{ product.category }}</el-tag>
      ...
      <div class="comments-section">
        ...
        <div class="comment-form">
          <el-input
            ...
            type="textarea"
            :rows="3"
            placeholder="Ask a question or leave a comment... (e.g., 'Is it fresh?')"
        ...
        <div class="comments-list" v-loading="commentsLoading">
          <el-empty v-if="comments.length === 0" description="No comments yet. Be the first to ask!" />

          <div v-for="comment in comments" :key="comment.id" class="comment-item">
            <div class="comment-header">
              <span class="comment-author">
                {{ comment.user ? comment.user.username : 'Unknown' }}
                <el-tag size="small" type="info" v-if="comment.user && comment.user.role">{{ comment.user.role }}</el-tag>
              </span>
